export default class Adapter {
				...
				.map(
					...
					<div class="card__item_card2">
						...
						<ul class="card__item_desc"> 
                            ${ul
								.slice(0, 2)
								.map((li, i, arr) => `<li> ${i === arr.length - 1 ? `${li}...` : li} </li>`)
								.join("")} 
                        </ul>
	...
	Agreement(container) {
		...
		<article class="agreement__body">
			...
			<h3>2 ВИДЫ ПЕРСОНАЛЬНЫХ ДАННЫХ </h3>
			<p class="agreement__body_text">
				В рамках настоящего Соглашения и Политики обработки персональных данных Оператор
				может получить от Пользователя следующую информацию, попадающую под категорию <strong> Общедоступные персональные данные </strong>:
			</p>
			<ul class="agreement__body_list">
				<li>ФИО;</li>
				<li>Телефон;</li>				
